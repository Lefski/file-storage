...
## 11.1. Смысл через имена: Имена должны сразу сообщать о назначении сущности. Избегайте непонятных аббревиатур.

<span style="color:green">Хорошо</span>
```
user_profile, calculate_total_price(), DatabaseConnection
```
...
## 11.2. Глаголы для функций/методов: Имена функций и методов должны быть глаголами или начинаться с глагола.

<span style="color:green">Хорошо</span>
```
get_user_data(), send_message(), is_valid()
```

# 11.3. Булевы переменные: Используйте префиксы is, has, can для ясности.
<span style="color:green">Хорошо</span>
```
...
Модульность на усмотрение разработчика. Нет требований к разделению кода на модули, но допускается различная степень модульности в разных частях проекта.

# 13. SOLID-принципы

SOLID-принципы в проекте применяются по желанию разработчика, они не являются обязательным требованием. Также можно исппользовать отдельные принципы по своему усмотрению.

# 14. Работа с внешними API и сервисами

Текущий проект является самодостаточным веб-приложением с собственной аутентификацией, файловым хранилищем и базой данных.
Интеграция с внешними API и сервисами на данный момент не предусмотрена в рамках данной реализации.